...
def test_store_raw1(conn):
    ...
    with closing(conn.cursor()) as cursor:
        ...
        datasource = name_to_datasource(cursor, "test_source001")
...
def test_store_raw_fractured_small(conn):
    ...
    with closing(conn.cursor()) as cursor:
        ...
        datasource = name_to_datasource(cursor, "test_source002")
    ...
    plugin.store_raw(datasource, granularity, timestamp,
    ...
    plugin.store_raw(datasource, granularity, timestamp,
            trend_names_part_2, raw_data_rows)
...
def test_store_raw_fractured_large(conn):
    ...
    with closing(conn.cursor()) as cursor:
        ...
        datasource = name_to_datasource(cursor, "test_source003")
    ...
    raw_data_rows_part_1 = [(dn_template.format(i), ("1", "2", "3"))
            for i in range(100)]
    ...
    raw_data_rows_part_2 = [(dn_template.format(i), ("4", "5", "6"))
            for i in range(100)]

...
class TestDataPackage(unittest.TestCase):
    def test_constructor(self):
        ...
        data_package = DataPackage(data_package_type, granularity, trends, rows)
    ...
    def test_merge_packages(self):
        ...
        data_package_1 = DataPackage(data_package_type, granularity, trends, rows)
        ...
        data_package_2 = DataPackage(data_package_type, granularity, trends, rows)
    ...
    def test_filter_trends(self):
        ...
        self.assertEqual(
            tuple(td.name for td in filtered_package.trend_descriptors), ("x", "z")
        )
    ...
    def test_split(self):
        ...
        group_dict = {"a": "blue", "b": "red", "c": "green", "d": "blue", "e": "red"}
        ...
        for color, package in packages:
            if color == "blue":
                self.assertEqual(
                    len(package.trend_descriptors),
                    2,
                    "blue package should have 2 trends",
                )
            elif color == "red":
                self.assertEqual(
                    len(package.trend_descriptors),
                    2,
                    "red package should have 2 trends",
                )
            elif color == "green":
                self.assertEqual(
                    len(package.trend_descriptors),
                    1,
                    "green package should have 1 trends",
                )

...
package_array_list_b = DataPackage(
    ["curve"], [(123001, TIMESTAMP, (["", ""],)), (123002, TIMESTAMP, (["", ""],))]
)


package_array_list_c = DataPackage(
    ["curve"],
    [(123001, (["e=34,c=1", "e=45,c=3", "e=33,c=2"],)), (123002, (["", "", ""],))],
)
...
class TestDataPackage(unittest.TestCase):
    ...
    def test_deduce_value_descriptors(self):
        ...
        self.assertEqual(
            value_descriptors[0], ValueDescriptor("power", datatype.registry["integer"])
        )
        self.assertEqual(
            value_descriptors[1], ValueDescriptor("height", datatype.registry["real"])
        )
        self.assertEqual(
            value_descriptors[2], ValueDescriptor("state", datatype.registry["text"])
        )
        self.assertEqual(
            value_descriptors[3],
    ...
    def test_deduce_data_types_array(self):
        ...
        self.assertEqual(
            attr_type_dict["curve"], ValueDescriptor("curve", datatype.registry["text"])
        )

    def test_deduce_data_types_empty(self):
        data_package = DataPackage(attribute_names=("height", "power", "refs"), rows=[])

        value_descriptors = data_package.deduce_value_descriptors()

        self.assertEqual(
            value_descriptors[0],
            ValueDescriptor("height", datatype.registry["smallint"]),
        )

    def test_to_dict(self):
        data_package = DataPackage(
            attribute_names=("height", "power"),
            rows=[(10034, TIMESTAMP, ["15.6", "68"])],
        )
        ...
        expected_json = {
            "attribute_names": ["height", "power"],
            "rows": [[10034, "2013-08-30T15:30:00+00:00", ["15.6", "68"]]],
        }
    ...
    def test_deduce_attributes(self):
        ...
        attr_dict = {attribute.name: attribute for attribute in attributes}

        self.assertEqual(attr_dict["power"].data_type, datatype.registry["integer"])
        self.assertEqual(attr_dict["height"].data_type, datatype.registry["real"])
        self.assertEqual(attr_dict["state"].data_type, datatype.registry["text"])
    ...
    def test_create_copy_from_lines(self):
        ...
        self.assertEqual(
            lines[0], "123001\t2013-08-30 15:30:00+00:00\t405\t0.0\ttrue\t\n"
        )
        ...
        self.assertEqual(
            lines[0], "123001\t2013-08-30 15:30:00+00:00\t{0,1,2,4,7,4,2,1,0}\n"
        )
        ...
        self.assertEqual(lines[1], "123002\t2013-08-30 15:30:00+00:00\t{0,1,2}\n")
        self.assertEqual(lines[2], "123003\t2013-08-30 15:30:00+00:00\t{\\N,\\N,\\N}\n")
        ...
        self.assertEqual(lines[0], "123001\t2013-08-30 15:30:00+00:00\t{\\N,\\N}\n")
        self.assertEqual(lines[1], "123002\t2013-08-30 15:30:00+00:00\t{\\N,\\N}\n")

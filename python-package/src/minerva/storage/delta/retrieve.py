...
from minerva.directory import helpers, relation
from minerva.db.generic import UniqueViolation
from minerva.db.postgresql import grant, create_schema
from minerva.directory.helpers import NoSuchEntityError, NoSuchEntityTypeError, \
	get_entity, create_entity, get_entitytype_by_id
...
def retrieve(conn, table, attribute_names, entities, ts=None):
	"""
	Retrieve all delta data or delta data for a specific timestamp.
	...
	if ts:
		...
		query = (
			...
			"JOIN ("
				...
				"AS sub "
			"ON sub.entity_id = d.entity_id AND sub.timestamp = d.timestamp").format(columns_part, table.render(), in_part)
	else:
...
def retrieve_related(conn, table_name, attribute_names, entities, relation_table_name, limit=None):
	"""
	Retrieve delta data for related entities.

	TODO: implement way to get delta data for a specific timestamp.
	"""
	where_parts = []
	join_parts = []

	full_base_tbl_name = "{0}.\"{1}\"".format(SCHEMA, table_name)
...
def retrieve_attributes_for_entity(conn, entity_id, attribute_ids):
	...
	with closing(conn.cursor()) as cursor:
		...
		for table_name, attrs_iter in groupby(rows, get_tablename):
			...
			# Get entitytype of first entry, All in same table, SO all same entitytype.
			target_entitytype_name = get_entitytype(attrs[0])
			if source_entitytype.name == target_entitytype_name:
				relation_table_name = "self"
			else:
				relation_table_name = "{}->{}".format(source_entitytype.name, target_entitytype_name)

			data = retrieve_related(conn, table_name, attr_names, [entity.id], relation_table_name)
			...
			for index, attr in enumerate(attrs):
				...
				result[attr_id] = [
					(to_unix_timestamp(get_timestamp(row)), prepare_value(get_attr_value(row))) for row in data]

...
from minerva.storage.delta.basetypes import AttributeStore, RawDataPackage
...
class DeltaPlugin(object):
	...
	def store_raw(self, datasource, raw_datapackage):
		if not raw_datapackage.is_empty():
			...
			naive_timestamp = datetime.strptime(raw_datapackage.timestamp, "%Y-%m-%dT%H:%M:%S")
			...
			self.store(datasource, entitytype, timestamp, raw_datapackage.attribute_names,
					refined_data_rows)

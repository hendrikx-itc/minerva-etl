...
from minerva.harvest.plugins import list_plugins, get_plugin as get_harvest_plugin
...
class LoadHarvestPlugin(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if plugin is None:
            print(f"Data type '{plugin_name}' not supported")
            sys.exit(1)
...
def show_rows_from_cursor(cursor, show_cmd=print):
    ...
    :return:
    """
    show_rows([c.name for c in cursor.description], cursor.fetchall(), show_cmd)

...
from minerva.commands import show_rows_from_cursor, show_rows
...
def create_attribute_store(conn, attribute_store: AttributeStore):
    query = (
        ...
        "ARRAY[{}]::attribute_directory.attribute_descr[]".format(
            ",".join(
                [
                    "('{}', '{}', '{}')".format(attribute.name, attribute.data_type, "")
    ...
    query_args = (attribute_store.data_source, attribute_store.entity_type)
...
def setup_remove_attribute_parser(subparsers):
    ...
    cmd.add_argument(
        "attribute_name", help="name of the attribute that should be removed"
    )
...
def remove_attribute_from_attribute_store_cmd(args):
    ...
    query_args = (args.attribute_name, args.attribute_store)
...
def setup_show_parser(subparsers):
    ...
    cmd.add_argument("--id", help="id of trend store", type=int)
...
class SampledViewMaterialization:
    ...
    @staticmethod
    def from_dict(data):
        return SampledViewMaterialization(data["attribute_store"], data["query"])
    ...
    def create(self, conn):
        ...
        query = sql.SQL('CREATE VIEW {} AS {}').format(
            sql.Identifier("attribute", view_name),
            self.query
        )
        ...
        with closing(conn.cursor()) as cursor:
            try:
                ...
                cursor.execute(
                    insert_materialization_query, insert_materialization_args
                )
            except Exception as exc:

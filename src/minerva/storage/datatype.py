    ...
    def deduce_parser_config(self, value: str) -> Optional[dict]:
        if value in self.bool_set:
            return merge_dicts(
                ...
                {"true_value": self.true_set, "false_value": self.false_set},
            )
        else:
            return None
...
class TimestampWithTimeZone(DataType):
    ...
    default_serializer_config = {"null_value": "\\N", "format": "%Y-%m-%dT%H:%M:%S"}
...
class Timestamp(DataType):
    default_parser_config = {"null_value": "\\N", "format": "%Y-%m-%dT%H:%M:%S"}
    ...
    def deduce_parser_config(self, value) -> Optional[dict]:
        ...
        for regex, datetime_format in self.known_formats:
            ...
            if match is not None:
                return merge_dicts(
                    self.default_parser_config, {"format": datetime_format}
                )
...
class SmallInt(DataType):
    ...
    default_parser_config = {"null_value": "\\N"}

    default_serializer_config = {"null_value": "\\N"}
    ...
    def deduce_parser_config(self, value: Any) -> Optional[dict]:
        ...
        if value == "":
            return merge_dicts(self.default_parser_config, {"null_value": ""})
...
class Integer(DataType):
    ...
    def _string_serializer_config(config):
        if config is None:
            ...
            return merge_dicts(Integer.default_serializer_config, config)
...
class Bigint(DataType):
    ...
    default_parser_config: Dict[str, str] = {"null_value": "\\N"}

    default_serializer_config: Dict[str, str] = {"null_value": "\\n"}
    ...
    def parse(cls, value):
        ...
        if not (cls.min <= int_val <= cls.max):
            raise ParseError(
                "{0:d} is not in range {1:d} - {2:d}".format(int_val, cls.min, cls.max)
            )
...
class DoublePrecision(DataType):
    ...
    def string_serializer_config(config):
        if config is None:
            ...
            return merge_dicts(DoublePrecision.default_serializer_config, config)
...
class Text(DataType):
    ...
    default_serializer_config = {"null_value": "\\N", "prefix": "", "postfix": ""}
...
class ArrayType(DataType):
    ...
    def string_parser_config(config: Optional[dict]) -> dict:
        ...
        return merge_dicts(ArrayType.default_string_parser_config, config)
    ...
    def string_serializer_config(config: Optional[dict]) -> dict:
        ...
        return merge_dicts(ArrayType.default_string_serializer_config, config)

    def string_parser(self, config: Optional[dict] = None):
        config = self.string_parser_config(config)

        base_type_parser = self.base_type.string_parser(config.get("base_type_config"))
    ...
    def string_serializer(self, config=None):
        ...
        def serialize(arr_value):
            if arr_value is None:
                ...
                return (
                    prefix
                    + separator.join(base_type_serializer(part) for part in arr_value)
                    + postfix
                )
...
TYPE_ORDER_RANKS: Dict[DataType, int] = dict(
    (data_type, i) for i, data_type in enumerate(TYPE_ORDER)
)
...
def max_data_types(
    current_data_types: Iterable[DataType], new_data_types: Iterable[DataType]
) -> List[DataType]:
    return [
        max_data_type(current_data_type, new_data_type)
        for current_data_type, new_data_type in zip(current_data_types, new_data_types)
    ]
...
def deduce_data_types(rows):
    ...
    return reduce(
        max_data_types,
        [
            [parser_descriptor_from_string(value).data_type for value in row]
            for row in rows
        ],
    )
...
copy_from_serializer_base_type_config = {
    registry["bigint"]: {"null_value": "\\N"},
    registry["boolean"]: {"null_value": "\\N"},
    registry["timestamp"]: {"null_value": "\\N"},
    registry["timestamp with time zone"]: {"null_value": "\\N"},
    registry["integer"]: {"null_value": "\\N"},
    registry["smallint"]: {"null_value": "\\N"},
    registry["real"]: {"null_value": "\\N"},
    registry["double precision"]: {"null_value": "\\N"},
    registry["numeric"]: {"null_value": "\\N"},
    registry["text"]: {"null_value": "\\N"},
}

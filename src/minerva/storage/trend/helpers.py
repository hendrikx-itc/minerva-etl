...
def get_previous_timestamp(timestamp, granularity):
    ...
    """
    previous = (timestamp.astimezone(pytz.utc) - timedelta(0, granularity)).astimezone(
        timestamp.tzinfo
    )
...
def get_next_timestamp(timestamp, granularity):
    ...
    :param granularity: granularity in seconds to subtract from timestamp
    """
    next = (timestamp.astimezone(pytz.utc) + timedelta(0, granularity)).astimezone(
        timestamp.tzinfo
    )
    ...
    try:
        ...
        if minerva_tz:
            ...
            return datetime.combine(
                most_recent,
                time(
                    most_recent.hour, most_recent.minute, most_recent.second, tzinfo=tz
                ),
            )
...
def get_table_names(trendstores, start, end):
    ...
    """
    return list(
        set(chain(*[trendstore.table_names(start, end) for trendstore in trendstores]))
    )


def get_table_names_v4(cursor, datasources, granularity, entitytype, start, end):
    """

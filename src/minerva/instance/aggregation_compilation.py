...
class AggregationContext:
    ...
    def generated_file_header(self) -> str:
        ...
        if path.is_absolute():
            relative_source_definition_path = self.instance.make_relative(path)
        else:
        ...
        if self.definition["entity_type"] != relation.target_entity_type:
            raise ConfigurationError(
                "Entity type mismatch between definition and relation target: {} != {}".format(
                    self.definition["entity_type"], relation.target_entity_type
                )
            )
...
def create_base_name_to_name_translation(aggregation_definition: dict):
    return {
        element["source"]: element["name"]
        for element in aggregation_definition["parts"]
    }
...
def compile_entity_aggregation(aggregation_context: EntityAggregationContext):
    ...
    if aggregation_type is EntityAggregationType.VIEW:
        trend_store = aggregation_context.instance.load_trend_store_by_name(
            aggregation_context.definition["source"]
        )
        relation = aggregation_context.instance.load_relation(
            aggregation_context.definition["relation"]
        )
        name_translation = create_base_name_to_name_translation(
            aggregation_context.definition
        )
...
def translate_source_part_name(
    aggregation_context: EntityAggregationContext, name: str
) -> str:
...
def generate_view_entity_aggregation(
    instance_root: Path,
    ...
    for part in trend_store.parts:
        part_name = name_translation.get(
            part.name,
            translate_entity_aggregation_part_name(
                part.name, relation.target_entity_type
            ),
        )
...
def define_aggregate_trend_store(
    ...
    trend_store.title = "Generated by Minerva aggregation generation command"

    return trend_store


def define_aggregate_part(data: TrendStorePart, definition):
    trends = [define_aggregate_trend(trend) for trend in data.trends]
    ...
    generated_trends = list(chain(data.generated_trends, added_generated_trends))
    ...
    return TrendStorePart(definition["name"], trends, generated_trends)
...
def write_time_aggregations(aggregation_context: TimeAggregationContext):
    ...
    for agg_part in aggregation_context.definition["parts"]:
        if "source" in agg_part:
            ...
            materialization_file_path = (
                aggregation_context.instance.materialization_file_path(agg_part["name"])
            )
            ...
            aggregate_definition = define_part_time_aggregation(
                source_part,
                aggregation_context.source_definition.granularity,
                mapping_function,
                target_granularity,
                agg_part["name"],
            )
    ...
    return OrderedDict(
        [
            ...
            (
                ...
                SqlSrc(
                    fingerprint_function_sql(
                        source_part, source_granularity, target_granularity
                    )
                ),
            ),
        ]
...
def aggregate_function(
    ...
    :param src_trend_store_part: The part that needs to be aggregated
    :param target_granularity:
    :return: definition of the function as an OrderedDict
    """
    trend_columns = [
        '  "{}" {}'.format(
            trend.name, aggregate_data_type(trend.data_type, trend.time_aggregation)
        )
        for trend in src_trend_store_part.trends
    ]
    ...
    if (
        len([trend for trend in src_trend_store_part.trends if trend.name == "samples"])
        == 0
    ):
    ...
    src = (
        ...
        + "\n"
        + '    FROM trend."{}" AS t\n'.format(src_trend_store_part.name)
...
def define_trend_store_link(
    part_name: str, mapping_function: str, target_name: str
) -> List[str]:
    return [
        ...
        "WHERE m::text = '{}' and tsp.name = '{}';\n".format(target_name, part_name),
    ]


def fingerprint_function_sql(
    part_data: TrendStorePart, source_granularity: str, target_granularity: str
) -> str:
    return (
